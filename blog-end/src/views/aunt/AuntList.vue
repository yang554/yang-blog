  ...
  <div class="user-list-view">
    ...
    >
      ...
      <el-table-column label="经血量" width="80px" align="center">
        <template slot-scope="scope"> {{ scope.row.bloodVolume }}ml </template>
      </el-table-column>
    ...
    <el-dialog title="本次实际记录" :visible.sync="dialogFormVisible">
      <el-form v-model="realAuntForm">
        ...
        <el-form-item label="周期长度" label-width="120px">
          ...
          <el-input
            class="el-form-item__label"
            v-model="realAuntForm.bloodVolume"
            autocomplete="off"
            style="width: 10%"
          ></el-input
          >ml
        ...
        <el-form-item label="开始日期" label-width="120px">
          <el-date-picker
            class="el-form-item__label"
            disabled
            v-model="realAuntForm.startDate"
            type="date"
            placeholder="选择开始时间"
          >
          </el-date-picker>
        ...
        <el-form-item label="症状" label-width="120px">
          <el-input
            ...
            type="textarea"
            :rows="5"
            placeholder="记录每个月姨妈期间出现的症状，如腹痛、乳房胀痛、情绪波动等"
            v-model="realAuntForm.symptom"
            style="width: 40%"
          ></el-input>
          ...
          <el-input
            class="el-form-item__label"
            type="textarea"
            placeholder="可以记录一些额外的信息，如服用的药物、健康状况等。"
            :rows="5"
            v-model="realAuntForm.note"
            style="width: 40%"
          ></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleSearchEvent() {
      if (this.searchName == "") {
        ...
        _getUserByName(this.searchName).then((res) => {
          if (res.data.obj.length > 0) {
            this.searchID = res.data.obj[0].id;
            _selectByName(this.searchID).then((res) => {
              if (res.data.status === 200) {
                this.pagination.listData = res.data.obj;
                this.pagination.total = res.data.obj.length;
